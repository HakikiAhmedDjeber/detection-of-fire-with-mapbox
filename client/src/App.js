...
import { GET_ALL_SENSSED_DATA, GET_ALL_SENSSED_DATA_BY_DEVICE } from "./GraphQL/queries";
...
export default function App() {
  ...
  const [getAllSenssedData, { loading: isLoading, data: responseData, error: queryError }] = useLazyQuery(GET_ALL_SENSSED_DATA); //  ADD new this will fetch allll historic
  const [getAllSenssedDataByDevice, { loading: isDLoading, data: DeviceResponseData, error: queryDeviceError }] = useLazyQuery(GET_ALL_SENSSED_DATA_BY_DEVICE); //  ADD new this will fetch all data by deviceID
  ...
  useEffect(() => {
    if (!isDLoading) {
      ...
      if (DeviceResponseData) {
        console.log("Recieved Data from single device ==> ", DeviceResponseData);
      }
    }
